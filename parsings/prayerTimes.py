...
# get by date
def get_prayer_times(currentMonth,regions, *currentDay):
    """Return the time of prayer for today"""
    url = f'https://islom.uz/vaqtlar/{regions}/{currentMonth}'

    response = requests.get(url).text
    soup = BeautifulSoup(response, 'html.parser')
    body = soup.find('tbody')
    head = soup.find('thead')
    region = soup.find("div",class_="row city_prayer_block")
    region = str(region.select("div")[0].text)
    region = region[15:]
...
def get_prayer_times_for_today(region=5):
    try:
        today = datetime.now().day
        currentMonth = datetime.now().month
        return get_prayer_times(currentMonth, region, today)
    except:
        return 'Бугун учун малумот топилмади!'

def get_prayer_times_for_tomorrow(region=5):
    try:
        today = datetime.now().day
        currentMonth = datetime.now().month
        if currentMonth == 2 and today == 28:
            return get_prayer_times(currentMonth+1, region, 1)
            ...
            return get_prayer_times(currentMonth, region, today+1)
    except:
        return 'Ертанки кун учун малумот топилмади!'


if __name__ == '__main__':
    print(get_prayer_times_for_tomorrow(5))
